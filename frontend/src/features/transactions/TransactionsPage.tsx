...
import { currentOperationalMonth, formatEconomicDate, monthPeriod } from '../../utils/dates'
...
export function TransactionsPage() {
  ...
  const [month, setMonth] = useState(initial.start.slice(0, 7))
  ...
  const query = useMemo<TransactionQuery>(() => ({ perspective, start_date: period.start, end_date: period.end, transaction_type: type || undefined, derived_status: status || undefined, category_id: categoryId ? Number(categoryId) : undefined, subcategory_id: subcategoryId ? Number(subcategoryId) : undefined, search: search.trim() || undefined, include_cancelled: includeCancelled || undefined, page, page_size: 10 }), [perspective, period, type, status, categoryId, subcategoryId, search, includeCancelled, page])
  ...
  useEffect(() => { setPage(1) }, [perspective, month, type, status, categoryId, subcategoryId, search, includeCancelled])
  ...
  return <div className="page-stack transactions-page">
    ...
    <Card className="period-panel"><div className="perspective-tabs" aria-label="Perspectiva financeira">{(['COMPETENCE', 'DUE', 'CASH'] as DatePerspective[]).map((item) => <button key={item} type="button" className={perspective === item ? 'active' : ''} aria-pressed={perspective === item} onClick={() => setPerspective(item)}>{PERSPECTIVE_LABELS[item]}</button>)}</div><div className="month-selector"><CalendarRange size={18} /><label htmlFor="transaction-month">Período</label><Input id="transaction-month" type="month" value={month} onChange={(event) => setMonth(event.target.value)} /><span>{period.label}</span></div></Card>
    ...
    {!loading && !error && data && data.items.length > 0 && <><div className="transaction-list" aria-label="Movimentações encontradas">{data.items.map((item) => <button type="button" className="transaction-card" key={item.id} onClick={() => void openDetail(item.id)}><div className="transaction-card__identity"><span className={`transaction-direction transaction-direction--${item.transaction_type.toLowerCase()}`}>{item.transaction_type === 'INCOME' ? 'Receita' : 'Despesa'}</span><strong>{item.description}</strong><small>{item.category.name}{item.subcategory ? ` · ${item.subcategory.name}` : ''}</small></div><div className="transaction-card__date"><span>{refDateLabel}</span><strong>{formatEconomicDate(item.reference_date)}</strong></div><dl><div><dt>Nominal</dt><dd>{formatMoneyBRL(item.amount)}</dd></div><div><dt>Liquidado</dt><dd>{formatMoneyBRL(item.settled_amount)}</dd></div><div><dt>Restante</dt><dd>{formatMoneyBRL(item.remaining_amount)}</dd></div></dl><StatusBadge tone={STATUS_TONES[item.derived_status]}>{STATUS_LABELS[item.derived_status]}</StatusBadge></button>)}</div><nav className="pagination" aria-label="Paginação"><Button variant="secondary" disabled={data.pagination.page <= 1} onClick={() => setPage((value) => value - 1)}><ChevronLeft size={17} />Anterior</Button><span>Página <strong>{data.pagination.page}</strong> de <strong>{data.pagination.total_pages}</strong> · {data.pagination.total_items} itens</span><Button variant="secondary" disabled={data.pagination.page >= data.pagination.total_pages} onClick={() => setPage((value) => value + 1)}>Próxima<ChevronRight size={17} /></Button></nav></>}
