...
def login(payload: LoginRequest, response: Response, session: Annotated[Session, Depends(get_session)], settings: Annotated[ApiSettings, Depends(get_api_settings)]) -> AuthSessionResponse:
    ...
    response.set_cookie(key=settings.session_cookie_name, value=created.token, max_age=settings.session_lifetime_hours * 3600, httponly=True, secure=settings.cookie_secure, samesite="none" if settings.environment == Environment.PRODUCTION else "lax", path=settings.api_prefix)
...
def logout(request: Request, response: Response, _current_user: Annotated[User, Depends(get_current_user)], session: Annotated[Session, Depends(get_session)], settings: Annotated[ApiSettings, Depends(get_api_settings)]) -> LogoutResponse:
    ...
    response.delete_cookie(settings.session_cookie_name, path=settings.api_prefix, secure=settings.cookie_secure, httponly=True, samesite="none" if settings.environment == Environment.PRODUCTION else "lax")
